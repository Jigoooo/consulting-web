...
PG18 migration/API runtime dual + PG-only preflight: ████████░░ 80% (API dual live; PG-only blocked by SQLite surface)
...
- PG18 migration Phase 8 API-runtime dual observation (2026-07-08)
  - Phase 7의 API-container blocker를 해소: PG18 sidecar를 `consulting-web_default`에 alias `pg18-rehearsal`로 연결하고 API image에 `psycopg[binary]` 추가.
  - API runtime env: `CONSULTING_BRAIN_BACKEND=dual`, `CONSULTING_BRAIN_WRITE_BACKEND=dual`, `CONSULTING_PG_DSN_DRIVER=psycopg`.
  - Real outbox→worker→Python ingest smoke 통과: marker `PHASE8_DUAL_SMOKE_20260708_194739`, SQLite `dialogue_chunks id=184`와 PG18 `brain_raw`/`brain_rag` readback 일치.
  - PG search ranking root fix: file trust tier가 relevance를 덮던 반례 수정(`weighted_score DESC, trust_group DESC`).
  - Short live observation: 약 33분/8샘플 clean(`healthy`, `pg18 ok`, parity true, mismatch 0, API error 0); Phase9 진단 중 의도적 shadow mismatch가 생긴 뒤 long watcher는 중단하고 fresh delta watcher로 분리.
  - 문서: `docs/pg18-migration-phase8-20260708.md`, `docs/pg18-migration-worklog.md`.

- PG18 migration Phase 9 PG-only preflight (2026-07-08)
  - API dual은 계속 정상: API health `healthy`, outbox `published=154`, non-published `[]`, SQLite↔PG18 consulting-web parity `(2,184,319)` 일치.
  - Direct PG marker recall 통과: marker `PHASE8_DUAL_SMOKE_20260708_194739`가 PG top `dialogue` chunk `184`로 검색됨.
  - PG-only는 보류: writer audit `writer_files=63`, `core_writer_files=4`, `sqlite_delete_safe=false`; reader/writer surface `sqlite_connect_files=39`, `runtimeish_sqlite_connect_files=32`.
  - 다음 단위는 live flip이 아니라 SQLite surface reduction / backend-aware read bridge / pg-only simulation.
  - 문서: `docs/pg18-migration-phase9-preflight-20260708.md`.
...
- PG18 Phase8 focused gate: `python3 -m py_compile scripts/dialogue_memory/pg_backend.py scripts/dialogue_memory/pg_store.py scripts/pg_migration/phase8_observe_api_dual.py`; `.venv/bin/pytest scripts/tests/test_dialogue_memory_backend_modes.py scripts/tests/test_dialogue_memory_write_backend_modes.py -q` → `20 passed`; prod + PG18 sidecar compose config OK
- PG18 Phase8 live gate: API container `pg18-rehearsal:5432` TCP/SQL OK, marker smoke SQLite↔PG18 readback OK, short observer 8/8 clean samples

## 다음 우선순위

1. PG18 Phase 10은 **SQLite surface reduction**부터 진행
   - core ingesters(`store.py`, `ingest.py`, `file_ingest.py`, `md_ingest.py`)의 pg-only guard test 추가.
   - runtime readers를 backend-aware read bridge로 분류/전환.
   - SQLite DB inaccessible fixture 또는 copy 기반 pg-only simulation으로 hidden dependency를 잡기 전까지 live flip 금지.

2. Advanced GraphRAG 7개 고도화는 새 세션에서 진행
