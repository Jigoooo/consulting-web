  ...
  return (
    <div>
      ...
      {items.map((e) => (
        <div key={e.id} className={`${s.card} ${hovered && e.messageId === hovered ? s.glow : ''}`}>
          <div className={s.cardHead}>
            <span className={s.srcIcon}><Icon name={sourceIcon[e.sourceType] ?? 'info'} size="sm" decorative /></span>
      ...
      {formOpen ? (
        ...
        <Button type="button" variant="ghost" size="sm" className={s.addBtn} leadingIcon="pin" onClick={() => setFormOpen(true)}>
          근거 추가
        </Button>
      )}
    </div>
