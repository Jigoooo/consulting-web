...
LEGACY_ROOT = Path('/home/jigoo/.hermes/workspace/consulting')
API_TESTS = [
...
LEGACY_TESTS = [
    str(LEGACY_ROOT / 'scripts/tests/test_dialogue_memory_graph_terms.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_cross_topic_links_persist.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_dialogue_memory_embedding_degradation.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_advanced_graphrag_write_guard.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_raptor_summary_layer.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_community_report_layer.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_tog2_deep_layer.py'),
    str(LEGACY_ROOT / 'scripts/tests/test_dialogue_memory_tog2_deep.py'),
]
...
def assert_static_contracts() -> list[str]:
    ...
    legacy_search = (LEGACY_ROOT / 'scripts/dialogue_memory/search.py').read_text(encoding='utf-8')
    legacy_phase3 = (LEGACY_ROOT / 'scripts/consulting_phase3_os.py').read_text(encoding='utf-8')
    legacy_advanced_guard = (LEGACY_ROOT / 'scripts/advanced_graphrag_write_guard.py').read_text(encoding='utf-8')
    legacy_advanced_layers = (LEGACY_ROOT / 'scripts/advanced_graphrag_layers.py').read_text(encoding='utf-8')
    ...
    if '_term_set(query)' not in legacy_search:
        failures.append('legacy graph search missing normalized term matching')
    if 'degraded_signals' not in legacy_search or '_degradation_error' not in legacy_search:
        failures.append('legacy recall missing embedding degradation observability')
    if 'tog2_deep' not in legacy_search or 'search_tog2_deep' not in legacy_search:
        failures.append('legacy recall missing ToG-2 deep signal integration')
    if 'insert into cross_topic_links' not in legacy_phase3:
        failures.append('legacy cross-topic suggestions are not persisted')
    for needle in ['CONSULTING_ADVANCED_GRAPHRAG_WRITE_APPROVED', 'CONSULTING_ADVANCED_GRAPHRAG_DEPS_APPROVED', 'source_chunk_ids']:
        if needle not in legacy_advanced_guard:
    ...
    for needle in ['write_raptor_summaries', 'write_community_reports', 'search_tog2_deep', 'source_chunk_ids', 'connected_components_no_dep']:
        if needle not in legacy_advanced_layers:
            failures.append(f'advanced GraphRAG layer missing {needle}')
    return failures


def legacy_db_contracts() -> dict:
