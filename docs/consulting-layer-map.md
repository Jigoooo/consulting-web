...
### 4.1 Scope 생성·lifecycle 원자성 (2026-07-11)

- 신규 채널의 `channel → 기본 topic → 첫 thread`는 `POST /spaces/channel-bundles` 한 transaction에서 생성한다. 기존 빈 채널도 `POST /spaces/channels/:id/ensure-conversation`으로 topic/thread/edge/outbox/audit를 한 번에 복구한다. Thread insert 실패 시 상위 생성까지 전부 rollback하며 동일·동시 재요청은 같은 IDs로 수렴한다.
- 사용자 보관은 `status='archived'`, 내부 soft-delete는 `status='deleted_soft' + deleted_at`이다. project/channel/topic/thread cascade와 graph edge/tag tombstone은 한 transaction으로 변경한다.
- `0035_scope_lifecycle_transition_ledger.sql`의 `scope_lifecycle_transitions`가 cascade 전 상태를 기록한다. Restore는 root row `FOR UPDATE` + optimistic `version` fence로 중복 요청을 한 단계만 소비하며, 각 descendant가 해당 event 이후 다른 lifecycle 상태로 변경됐으면 replay하지 않아 독립 보관·soft-delete를 되살리지 않는다.
- 기본 read/access 경로는 요청 scope뿐 아니라 workspace→project→channel→topic 전체 ancestor의 tenant 일치, `status='active'`, `deleted_at is null`을 확인한다.
...
### 7.2 exact-thread Hermes memory

Hermes run session은 Web의 exact thread 단위로 묶인다.
...
```text
- 화면 transcript와 Hermes 대화 상태 모두 thread별 격리.
- 같은 고객사/프로젝트/채널이라는 이유만으로 sibling thread 기억을 강제 공유하지 않음.
- 관계는 context graph 메타데이터로만 보존하고, cross-scope recall은 명시 allow-list가 있을 때만 수행.
- General/default는 자동 recall 없는 검토 대기 공간.
```
...
### 18.2.1 W2-5 artifact red-team warning layer — local pre-deploy

```text
POST /artifacts/:id/verify가 exact verifier PASS를 기록
  -> artifact_red_team_jobs + outbox_events 원자적 enqueue
  -> outbox relay -> Redis/Bull artifact-red-team queue
  -> leased worker가 현재 content + governing_message + so_what hash 재계산
  -> 전용 Hermes reviewer GET /v1/toolsets가 complete effective inventory=0임을 attestation
  -> 감사원·의회·노조 3-persona strict JSON review
  -> fenced terminal job update + append-only artifact_red_team_runs 1:1 insert
  -> preflight/UI가 pending|processing을 polling하고 warning만 표시
```

불변식:

```text
- runtime 기본값은 off다. shadow/warning은 전용 reviewer URL/key가 없으면 앱 부팅부터 실패한다.
- main chat Hermes endpoint/key는 reviewer에 재사용하지 않는다. reviewer endpoint가
  `inventory_complete=true`, `effective_toolsets=[]`, `effective_tools=[]`를 positive attestation하지 않으면
  /v1/runs를 시작하지 않는다.
- Hermes `/v1/toolsets`는 default MCP를 숨길 수 있으므로 전용 profile은 `no_mcp`를 명시하고,
  activation 전에 실제 `_get_platform_tools(config, "api_server")` 결과가 빈 집합임을 별도 확인한다.
- job은 tenant + artifact/version + SHA-256(content+structure) + policy로 idempotent하다.
- lease token·heartbeat·bounded retry·expired-lease recovery가 stale worker 완료를 차단한다.
- terminal run은 job_id NOT NULL/UNIQUE이며 job 상태·tenant/hash/policy와 DB trigger로 일치한다.
- run은 UPDATE/DELETE 불가, 부모 FK RESTRICT, trigger search_path 고정이며 reviewer 사용자 삭제 후에도
  UUID audit attribution을 보존한다.
- instruction-like input은 reviewer에 전달하지 않고 3-persona deterministic BLOCKED run으로 남긴다.
- red-team 장애·stale·missing·non-pass는 warning 단계에서 사용자에게 보이되 export를 차단하지 않는다.
- cached verifier PASS는 background red-team polling 중에도 export 버튼을 막지 않는다.
```

Schema/API:

```text
migration: packages/db-schema/drizzle/0036_artifact_red_team_runs.sql
tables: artifact_red_team_jobs, artifact_red_team_runs
contract: ArtifactExportPreflightResponse.redTeam
UI: ArtifactsSurface warning + 1.5s pending/processing polling
status: local test candidate; production migration/mode/profile 미적용, Hermes attestation support 대기
```
