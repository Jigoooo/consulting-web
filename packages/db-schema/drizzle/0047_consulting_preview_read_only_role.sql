-- Dedicated least-privilege role for V3-5 read-only preview snapshots.
-- The application login may SET ROLE only after PGOPTIONS has already enabled
-- default_transaction_read_only at connection startup.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'consulting_preview_ro') THEN
    CREATE ROLE consulting_preview_ro
      NOLOGIN
      NOSUPERUSER
      NOCREATEDB
      NOCREATEROLE
