...
| # | 결정 | 값 |
|---|---|---|
| D3 | **사용자 노출 UI** | **"보관하기"만 노출.** 삭제/휴지통/영구삭제 개념은 UI에 없음 → 사용자 입장에선 데이터가 절대 사라지지 않고 보관/복원만 존재. |
| D4 | **보관기간** | **무기한.** archived·deleted_soft 둘 다 만료 없음. 자동 purge 잡 없음(retention·물리삭제 후보 로직 제거). |
| — | **내부 상태** | 내부적으로 `active`/`archived`/`deleted_soft` 3-state는 유지하되, `deleted_soft`·물리삭제 경로는 **UI 진입점 없이 코드에만**(향후 관리자 도구용). |

**해석:** 사용자 관점 = "보관하기"(트리 접힘, 검색·참조 계속) ↔ "복원"뿐. 데이터 소멸 없음. `deleted_soft`는 이번 스코프에서 UI 미노출, 코드 골격만 남겨 향후 관리자 전용으로 확장.
...
|---|---|---|---|---|---|---|
| `active` | O | O | O | O | — | 존재 |
| `archived` | 접힘/보관함 | O | O(약하게) | 읽기전용 | 즉시(복원) | 존재 |
| `deleted_soft` | X | X | X(tombstone) | X | 무기한(관리자 전용) | 존재 |
| (물리삭제) | — | — | — | — | 불가 | 워크스페이스 cascade만 |

> 사용자 UI는 `active ↔ archived`만 오간다(보관/복원). `deleted_soft`는 UI 미노출.

**규칙:**
1. `archived` = "일 끝난 채널, 지식은 보존". 주인님의 "남아있어야 한다"에 정확히 대응. 검색·참조 가능, 트리에서만 접힘. **사용자에게 노출되는 유일한 삭제 대체 동작(D3).**
2. `deleted_soft` = 내부 전용 상태. 트리/검색/참조 전부 제외. **이번 스코프에선 UI 진입점 없음**(관리자 도구 확장 시 사용). deleted_at은 tombstone 기준시각으로만.
3. **보관기간 무기한(D4).** archived·deleted_soft 둘 다 만료 없음. 자동 purge 없음. 물리삭제는 워크스페이스 통삭제 시 FK cascade로만.
4. 아티팩트·evidence·chat_messages는 노드가 archived/deleted_soft여도 **보존**(감사·재활용). 트리 표시에서만 숨김.
...
### Task B5: 보관함 복원 API (retention·purge 제거 — D4)
- **Files:** `apps/api/src/spaces/archive.controller.ts`(`GET /archive`, `POST /archive/:id/restore`) + test
- `/archive`: 현재 워크스페이스의 archived 노드 목록(복원 후보). **만료·기간 개념 없음(무기한).**
- 복원 = `restoreNode`로 active 전환. 부모 live 검증(부모가 archived면 "먼저 상위부터 복원" 안내).
- **purge 잡 없음.** 자동 물리삭제 로직은 이번 스코프에서 만들지 않음(D4). deleted_soft·물리삭제는 향후 관리자 도구 트랙.
- Step1(RED): "archived 노드가 /archive에 뜬다", "restore→active복귀", "부모 archived 상태서 자식 restore→상위복원 안내".

### Task B6: 프론트 — "보관하기" + 보관함 UI (D3, 휴지통 개념 없음)
- **Files:** `apps/web/src/features/archive/*` + 사이드바 진입점
- 프로젝트/채널 우클릭·… 메뉴에 **"보관하기"** 단일 액션(삭제·휴지통 문구 없음). 확인 다이얼로그: "이 채널을 보관합니다. 검색·참조는 계속 가능하고, 언제든 복원할 수 있어요."
- 사이드바 하단 **"보관함"** 진입점 → archived 목록 + "복원" 버튼.
- **삭제/영구삭제/휴지통 UI는 만들지 않음(D3).** 데이터 소멸 경로를 사용자에게 노출하지 않음.
- Step: 계약테스트 + 최소 UI(saas-ui-ux 라운드 규칙: 확인 다이얼로그·로딩·다크·hover 전수).
...
- **restore의 상향 의존:** 자식만 복원했는데 부모가 archived면 트리 안 보임. → restore는 부모 live 검증, 아니면 "상위부터 복원" 안내.
...
- **데이터 소멸 없음(D4):** 자동 purge·물리삭제 UI 없음 → "완전삭제 회피" 완전 준수. 유일한 물리삭제는 워크스페이스 통삭제(FK cascade)뿐.
- **무기한 보관의 비용:** archived/deleted_soft가 영원히 쌓임 → 장기적으로 테이블 팽창 가능. 현재 규모(수십 행)에선 무시. 향후 관리자 도구에서 선택적 purge를 별도 트랙으로.

---

## 6. 결정 완료 · 남은 튜닝 · 고도화 (주인님 요청)

### 결정 완료 (§0.5)
- ✅ 사용자 UI = "보관하기"만, 삭제/휴지통 개념 없음(D3)
- ✅ 보관기간 무기한, 자동 purge 없음(D4)
- ✅ 내부 3-state 유지하되 deleted_soft·물리삭제는 UI 미노출(관리자 트랙)

### 남은 규칙 명문화 (구현 시 확정)
1. **자식↔부모 상태 조합:** 프로젝트 active인데 채널만 archived = O(정상). 프로젝트 archived면 하위 전부 archived cascade. active 자식이 archived 부모 밑에 남는 조합은 금지(cascade로 방지).
2. **아티팩트/evidence 운명:** 노드 보관 시 산출물도 트리에서 숨기되 **DB 보존 + 검색 가능**(재활용). 별도 "보관된 산출물" 뷰는 고도화(H2).
3. **보관 채널의 채팅:** 읽기전용(새 메시지 금지). 복원해야 대화 재개. (UI에서 입력창 비활성 + "복원하기" CTA)

### 고도화 로드맵
- **H1. audit 기반 타임라인:** 이미 `audit_events` 있음 → 노드별 "생성/보관/복원" 이력 타임라인 UI. 컨설팅 감사에 직결.
- **H2. 보관함 통합 뷰:** 워크스페이스 전역 "보관된 항목" 대시보드(프로젝트·채널·산출물 한눈에, 복원 UX).
- **H3. cascade 미리보기:** 프로젝트 보관 전 "채널 3·토픽 5·스레드 12개가 함께 보관됩니다" 확인 다이얼로그(파괴적 작업 투명성 — 주인님 "실패 투명" 선호).
- **H4. 엣지 GC 정기 리포트:** dead edge/tag 수를 주기 스캔해 리포트(비침습 read-only analyzer — 주인님 선호 패턴). 자동정리 아님, 가시화만.
- **H5. 복원 시 그래프 재추론:** restore된 채널에 A의 infer-related 잡 재실행 → 그동안 생긴 형제·타프로젝트와 재연결.
- **H6. 관리자 전용 purge 트랙(별도):** 무기한 보관이 언젠가 부담되면, 관리자만 접근하는 deleted_soft 영구삭제 도구를 별도 스코프로. 사용자 UI엔 영원히 미노출.
